...
export function getAllCategories(pageRequested, pageSize) {
  return axios
    .get('/categories', { params: { pageRequested, pageSize } })
    .then(res => {
      const { data } = res;
      return data;
    });
...
export function getCategoriesByFilter(
  searchType,
  searchKeyword,
  pageRequested,
  pageSize,
...
export function addBusinessToCategoryById(addedBusinessInfo, categoryID) {
  return axios
    .post(`/categories/${categoryID}/businesses/${addedBusinessInfo}`)
    .then(res => {
...
export function deleteBusinessFromCategoryById(deletedBusinessID, categoryID) {
  return axios.delete(
    `/categories/${categoryID}/businesses/${deletedBusinessID}`
  );
}

export function getBusinessByName(name) {
  return axios.get('/businesses', { params: { key: name } }).then(res => {
    const { data } = res;
    return data;
  });
}
...
export function getBusinessByFilter(searchType, searchKeyword) {
  ...
  return axios
    .get('/businesses', { params: { searchType, searchKeyword } })
    .then(res => {
      console.log(res);
      const { data } = res;
      return data;
